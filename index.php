...
<body>
<h1>pdf_text_copy_guard_canceller-webapp</h1>
...
<details>
	...
	<li>テキストコピーガードを解除したいPDFをzipしてアップロードしてください（複数ファイルok）。解除後のPDFがzipされて返ってきます。</li>
	<br/>
	<b>Note:</b>
	確実にすべてのテキストのコピーガードが解除されるとは限りません。
</details>

<?php
$cwd = getcwd();
$path = './temp';
$pdf_text_copy_guard_canceller_cmd = './pdf_text_copy_guard_canceller-cmd.exe';
...
function mainProcess($file_fullpath, $path, $folder, $filename, $cwd){
	if (move_uploaded_file($_FILES['file']['tmp_name'], $file_fullpath)) {
		...
		shell_exec("pdf_text_copy_guard_canceller-cmd.exe $proc_folder"); // Run pdf_text_copy_guard_canceller-cmd.exe
